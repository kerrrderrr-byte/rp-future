...
def load_users_local():
    if os.path.exists(USERS_FILE):
        try:
            ...
            pass
    default = {'ADMIN': {'username': 'ADMIN', 'login': 'ADMIN', 'password_hash': hash_password('qwerty123'),
                         'display_name': 'Администратор', 'is_admin': True}}
    ...
    if IS_PRODUCTION and supabase:
        try:
            supabase.table('users').insert(
                {'username': username, 'login': login, 'password_hash': hash_password(password),
                 'display_name': display_name, 'is_admin': False}).execute()
        ...
        users[login] = {'username': username, 'login': login, 'password_hash': hash_password(password),
...
def get_worlds_list():
    if supabase:
        try:
            ...
            pass
    return [{'world_id': 'academy_sakura', 'name': 'Академия Сакура', 'description': 'Школьная повседневность',
             'genre': 'school'}]
        ...
        try:
            ...
            if world.data:
                ...
                for c in (characters.data or []):
                    chars_dict[c['char_id']] = {
                        'id': c['char_id'], 'name': c['name'],
                        'role': c.get('role', ''), 'personality': c.get('personality', ''),
                        'speaking_style': c.get('speaking_style', ''),
                        'text_color': c.get('text_color', '#fff'),
                        'emoji': c.get('emoji', ''), 'sprite': c.get('sprite_name'),
                        'position': c.get('default_position', 'center')
                    }

                locs_dict = {}
                for l in (locations.data or []):
                    locs_dict[l['location_id']] = {
                        'name': l['name'], 'description': l.get('description', ''),
                        'mood': l.get('mood', ''), 'bg_image': l.get('bg_image', '')
                    }
            ...
            print(f"⚠️ World load error: {e}")

    # Fallback
    return {
        'world_name': 'Академия Сакура',
        'characters': {},
        'locations': {'classroom': {'name': 'Класс'}},
        'rules_for_ai': '{"speaker_text":"Привет"}',
        'first_scene_prompt': 'Начинаем игру.'
    }
...
def save_game_supabase(game_data):
    ...
    try:
        ...
        if existing.data:
            supabase.table('game_sessions').update(data).eq('session_id', game_data['session_id']).execute()
        else:
...
def call_deepseek(messages, max_tokens=800):
    ...
    try:
        resp = requests.post(DEEPSEEK_API_URL, headers={'Authorization': f'Bearer {DEEPSEEK_API_KEY}',
...
def get_test_response():
    import random
    scene = random.choice([
        {"time_of_day": "morning", "response_type": "dialog_chain", "narrator_text": "Солнечный свет заливает класс.",
         "speaker_id": "monika", "speaker_name": "Моника", "speaker_text": "Привет! Добро пожаловать!",
         "emotion": "happy", "location": "classroom", "dialog_end_marker": "direct_question_to_player",
         "dialog_chain": [{"speaker": "monika", "text": "Привет! Ты новенький? Я Моника.", "emotion": "happy"},
                          {"speaker": "garfild", "text": "Моника, дай человеку освоиться.", "emotion": "serious"},
                          {"speaker": "monika", "text": "Гарфилд, ты как всегда зануда!", "emotion": "excited"}],
         "sprites": {"garfild": {"visible": True, "position": "left", "highlight": False, "emotion": "normal"},
                     "monika": {"visible": True, "position": "center", "highlight": True, "emotion": "happy"},
                     "reiko": {"visible": False}, "yuki": {"visible": False}, "takeshi": {"visible": False},
                     "hana": {"visible": False}, "haru": {"visible": False}, "akira": {"visible": False},
                     "sensei": {"visible": False}, "yumi": {"visible": False}, "ryuu": {"visible": False},
                     "emi": {"visible": False}}}
    ])
    scene['_is_test_response'] = True
    return scene
...
def api_register():
    ...
    if len(login) < 3 or len(password) < 4: return jsonify(
        {'success': False, 'error': 'Минимум 3 символа логин, 4 пароль'}), 400
...
def api_login():
    ...
    resp = jsonify({'success': True,
                    'user': {'username': user['username'], 'display_name': user.get('display_name', user['username']),
...
def api_me():
    ...
    return jsonify({'success': True,
                    'user': {'username': user['username'], 'display_name': user.get('display_name', user['username']),
                             'is_admin': user.get('is_admin', False)}}) if user else jsonify({'success': False}), 401
...
def create_game():
    ...
    wdb = get_world_data(wid)
    sid = datetime.now().strftime('%Y%m%d_%H%M%S') + '_' + uuid.uuid4().hex[:4]
    msgs = [{'role': 'system', 'content': wdb.get('rules_for_ai', '')},
            {'role': 'user', 'content': wdb.get('first_scene_prompt', '').replace('{player_name}', name)}]
    ...
    game_data = {
        'session_id': sid, 'player_name': name, 'world_id': wid,
        'created_at': datetime.now().isoformat(), 'status': 'active',
        'current_location': scene.get('location', 'classroom'),
        'game_history': [scene], 'relationships': relationships,
        'ai_context': {'world_id': wid,
                       'messages': msgs + [{'role': 'assistant', 'content': json.dumps(scene, ensure_ascii=False)}],
                       'discovery': {}}
    }
    save_game(game_data)
    ...
    if token and supabase and IS_PRODUCTION:
        try:
            supabase.table('player_saves').insert({
                'user_login': token.split(':')[0], 'session_id': sid, 'player_name': name,
                'world_id': wid, 'world_name': wdb.get('world_name', ''),
                'current_location': scene.get('location', 'classroom'),
                'last_scene_preview': (scene.get('speaker_text', '') or scene.get('narrator_text', ''))[:100],
                'game_history': json.dumps([scene]), 'ai_context': json.dumps(game_data['ai_context']),
                'relationships': json.dumps(relationships), 'status': 'active'
            }).execute()
        except Exception as e:
...
def game_action():
    ...
    if 'relationship_changes' in scene and scene['relationship_changes']:
        ...
        for char_id, change in scene['relationship_changes'].items():
            ...
            game['relationships'][char_id] = max(-100, min(100, game['relationships'].get(char_id, 0) + change))
    ...
    if 'sprites' in scene:
        ...
        for char_id, sprite_data in scene['sprites'].items():
            ...
            if sprite_data.get('visible') and not game['ai_context']['discovery'].get(char_id, {}).get('unlocked'):
                game['ai_context']['discovery'][char_id] = {'unlocked': True, 'viewed': False,
...
def api_saves_list():
    ...
    if IS_PRODUCTION and supabase:
        try:
            r = supabase.table('player_saves').select('*').eq('user_login', ul).order('last_updated',
                                                                                      desc=True).execute()
            saves = r.data or []
        except:
            pass
        ...
        for sid, g in load_games_local().items():
            saves.append({'session_id': sid, 'player_name': g.get('player_name', ''), 'world_id': g.get('world_id', ''),
                          'world_name': g.get('world_name', ''), 'current_location': g.get('current_location', ''),
                          'last_updated': g.get('created_at', ''), 'created_at': g.get('created_at', ''),
                          'last_scene_preview': g.get('last_scene_preview', ''), 'status': g.get('status', 'active')})
...
def api_sync_save():
    ...
    if history:
        last = history[-1]
        last_scene = ('🗣️ ' + last.get('player_text', '')) if last.get('type') == 'player_action' else (
                    last.get('speaker_text', '') or last.get('narrator_text', ''))

    if IS_PRODUCTION and supabase:
        try:
            sd = {
                'user_login': token.split(':')[0], 'session_id': session_id,
                'player_name': game['player_name'], 'world_id': game.get('world_id', 'academy_sakura'),
                'world_name': game.get('world_name', ''), 'current_location': game.get('current_location', ''),
                'last_scene_preview': last_scene[:100],
                'game_history': json.dumps(game.get('game_history', [])),
                'ai_context': json.dumps(game.get('ai_context', {})),
                'relationships': json.dumps(game.get('relationships', {})),
                'status': game.get('status', 'active'), 'last_updated': datetime.now().isoformat()
            }
            existing = supabase.table('player_saves').select('id').eq('session_id', session_id).execute()
            if existing.data:
                supabase.table('player_saves').update(sd).eq('session_id', session_id).execute()
            else:
                supabase.table('player_saves').insert(sd).execute()
        except Exception as e:
...
def api_encyclopedia():
    session_id = request.args.get('session', '')
    game = get_game(session_id)
    wdb = get_world_data('academy_sakura')
    ...
    for cid, c in chars.items():
        ...
        result[cid] = {
            'id': cid, 'name': c.get('name', cid), 'emoji': c.get('emoji', ''),
            'sprite': c.get('sprite', ''), 'text_color': c.get('text_color', '#fff'),
            'role': c.get('role', ''), 'personality': c.get('personality', ''),
            'unlocked': cd.get('unlocked', False), 'viewed': cd.get('viewed', False),
            'relationship': relationships.get(cid, 0)
        }
...
def health():
    return jsonify({'status': 'ok', 'mode': 'production' if IS_PRODUCTION else 'local', 'api': bool(DEEPSEEK_API_KEY),
